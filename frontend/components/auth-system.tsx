  ...
  return (
    <div className="relative md:col-span-1">
      <input
        ...
        onFocus={onFocus}
        onBlur={onBlur}
        className={`w-full px-4 py-3 bg-input text-foreground border ${error ? "border-destructive" : "border-border"} rounded-lg focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-300`}
        aria-invalid={!!error}
...
const SelectField = ({ id, name, value, onChange, error, disabled, children, colSpan = "md:col-span-2" }: SelectFieldProps) => (
  <div className={`relative ${colSpan}`}>
    <select
      ...
      onChange={onChange}
      disabled={disabled}
      className={`w-full px-4 py-3 bg-input text-foreground border ${error ? "border-destructive" : "border-border"} rounded-lg focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-300 appearance-none disabled:opacity-50 disabled:cursor-not-allowed`}
      aria-invalid={!!error}
      aria-describedby={error ? `${id}-error` : undefined}
    >
      {children}
...
const LoginPage = ({ onSwitchToCreate, handleAuthSuccess }: LoginPageProps) => {
  ...
  return (
    <div className="w-full max-w-md p-8 space-y-6 bg-card text-card-foreground rounded-2xl shadow-lg animate-fade-in">
      ...
      <p className="text-sm text-center text-muted-foreground">
        ...
        <button onClick={onSwitchToCreate} className="font-semibold text-primary hover:underline">
          Sign Up
        </button>
      </p>
    </div>
...
const CreateAccountPage = ({ onSwitchToLogin, handleAuthSuccess }: CreateAccountPageProps) => {
  ...
  const validateForm = () => {
    ...
    formData.serviceLocations.forEach((loc, index) => {
      ...
      if (Object.keys(locError).length > 0) { locationErrors[index] = locError }
    })
  ...
  return (
    <div className="w-full max-w-3xl p-8 space-y-6 bg-card text-card-foreground rounded-2xl shadow-lg animate-fade-in">
            ...
            <div key={index} className="p-4 border border-border rounded-lg space-y-4">
              <div className="flex justify-between items-center">
                ...
                {formData.serviceLocations.length > 1 && (
                  <button type="button" onClick={() => handleRemoveLocation(index)} className="px-2 py-1 text-xs text-destructive hover:bg-destructive/10 rounded">Remove</button>
                )}
              </div>
              <div className="flex flex-col space-y-4">
                <SelectField id={`state-${index}`} name="state" value={location.state} onChange={(e) => handleLocationChange(index, e)} error={errors.serviceLocations?.[index]?.state} disabled={isLoading}>
                ...
                <SelectField id={`city-${index}`} name="city" value={location.city} onChange={(e) => handleLocationChange(index, e)} error={errors.serviceLocations?.[index]?.city} disabled={isLoading || !location.district}>
                  ...
                  {(cities[index] || []).map((city) => (<option key={city} value={city}>{city}</option>))}
                </SelectField>
              </div>
            </div>
          )
          )}
          <button type="button" onClick={handleAddLocation} disabled={isLoading} className="w-full px-4 py-2 text-sm font-semibold text-secondary-foreground bg-secondary rounded-lg hover:bg-secondary/90 focus:outline-none focus:ring-4 focus:ring-ring/50 transition-all duration-300 transform hover:-translate-y-1">
            + Add Location
          </button>
        </div>
      ...
      <p className="text-sm text-center text-muted-foreground">
        ...
        <button onClick={onSwitchToLogin} className="font-semibold text-primary hover:underline">Sign In</button>
      </p>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-background relative flex items-center justify-center font-sans p-4 overflow-hidden">
      ...
      <div className="relative w-full z-10">
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold text-foreground tracking-wider animate-fade-in-down drop-shadow-lg" style={{ animationDelay: "0.1s" }}>
            CrisisConnect
          </h1>
          <p className="text-foreground text-lg mt-2 animate-fade-in-down drop-shadow-md" style={{ animationDelay: "0.3s" }}>
